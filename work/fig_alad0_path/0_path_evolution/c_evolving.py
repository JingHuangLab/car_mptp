...
if __name__ == '__main__':
  ...
  path_fix_mode = 'none' if int(dir_output.split('_')[-1]) <= 10 else 'both' # 'head' if int(dir_output.split('_')[-1]) <= 5 else 'none' if int(dir_output.split('_')[-1]) <= 10 else 'both'
  ...
  car = CAR(cons_regulr_curv_thresh=30., cons_convergence_maxitr=100, )
  car.implement(fn_get_path_tangent=external_functions[2], 
                fn_get_path_weighted_rms=external_functions[3], )
  
  evolver = VoronoiConfinedPathEvolver(config_path_fix_mode=path_fix_mode)
