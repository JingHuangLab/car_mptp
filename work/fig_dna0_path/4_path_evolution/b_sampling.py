# Transition PathCV optimization on DNA WC HG transition: Sample the boxes.
# Zilin Song, 20231213
# 
...
from helpers import get_openmm_system_and_integrator, OpenMMPlatform, PDBFile, get_openmm_topology
...
if __name__ == '__main__':
  ...
  if whoami == 0:
    print(np.linalg.norm(path_colvar[1:  , :] - path_colvar[0:-1, :], axis=1), flush=True)
  ...
  replica.initialize_context_velocities(temperature=300.)
  ...
  sampler.md_execute_sampling(num_batches=5_000,
  ...
  with open(os.path.join(dir_output, f'chk_replica{str(whoami)}_coordinates.pdb'), 'w') as fo:
    PDBFile.writeModel(topology =get_openmm_topology(),
                       positions=replica._openmm_context.getState(getPositions=True, enforcePeriodicBox=True).getPositions(asNumpy=True),
                       file=fo, )
